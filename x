...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        -- Disable cam lock if in third-person mode
        if not IsFirstPerson() then
            -- Third-person mode: target the player but no cam lock
            ...
            if BodyPart then
                ...
                if predictedPosition then
                    -- No cam lock, just position tracking
                    Camera.CFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)
                end
            end
        else
            -- First-person mode: apply cam lock and other effects
            if TargetPlayer.Character:FindFirstChildOfClass("Humanoid").Health < 1 then
                TargetPlayer = nil
                IsTargeting = false
                return
            ...
            if BodyPart then
                ...
                if predictedPosition then
                    -- Apply cam lock in first-person mode
